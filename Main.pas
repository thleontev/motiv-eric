...
type
  TFMain = class(TForm)
    ...
    procedure UpdateDataForm; // считываем данные с формы  во внутренние структуры
    procedure UpdateForm; // проверка корректности заполенения форм
  ...
  TRbs = Record
              Name: string[10];
          CountTRU: integer;   // количество трансиверов
              TRU  : array [0..11] of integer;   // 0 - TRU не подключен, 1-6 подключен
      BlockName: string[5]; // DRU, CDU, RRU
   CountTRUInBlock: integer;    // количество трансиверов в блоке
              PWRB: integer;    // max мощность
               PWR: integer;    // мощность в режиме HYB
              SENS: integer;    // чувстивтельность - пока не используется
  end;
  ...
  TTransport = Record
              ...
              Sat: boolean;        // работаем через ЗССС
              // SIU
              ePSTU: String;
              cbSCNUM: String;
              eSCGR: String;
  ...
  TLAI =  Record
      Data: array[0..99,0..9] of String; // параметры LAI
      ColCount: integer;
      RowCount: integer;
  end;

  TBSC =  Record
      Data: array[0..20,0..5] of String; // параметры BSC
      ColCount: integer;
      RowCount: integer;
  end;

  TCodec =  Record
      Data: array[1..12,0..6] of String; // параметры BSC
      ColCount: integer;
      RowCount: integer;
  end;
...
var
  FMain: TFMain;
  LSector: array [1..6] of TSector;  // параметры секторов
  LRbs: array [1..10] of TRbs;       // параметры RBS
  LOptions: TOptions;                // общие праметры
  LTransp:TTransport;                // параметры транспорта
  LLAI: TLAI;
  LBSC: TBSC;
  LCodec: TCodec;
  CountRBSType: integer;             // хз
  cmdList: TStringList;              // спискок для обработки блочных команд
  DCPTRU: array[0..11,1..3] of String; // таблица соотвествие подключения DCP к TRX

  StrGrid: array[0..99,0..9] of String; // таблица для разбора CVS файла
...
datachek:=true;
alarmmsg:='Внимание!'+#13#10+#13#10;

// проверка конф. по трансиверам
nc:=0;
for n:=1 to 6 do
begin
...
if nc=0 then begin
   alarmmsg:=alarmmsg+'- не определена конфигурация по TRX'+#13#10;
   datachek:=false;
end;
// проверка TG
if eTG.Text='' then begin
   alarmmsg:=alarmmsg+'- не задан TG'+#13#10;
   datachek:=false;
end;
// проверка SC
if eSCGR.Text='' then begin
   alarmmsg:=alarmmsg+'- не задан SCGR'+#13#10;
   datachek:=false;
end;
// проверка PSTU
if eSCGR.Text='' then begin
   alarmmsg:=alarmmsg+'- не задано имя PSTU'+#13#10;
   datachek:=false;
end;
// проверка SITE
if eSITE.Text='' then begin
   alarmmsg:=alarmmsg+'- не задано имя SITE'+#13#10;
   datachek:=false;
end;
// проверка RSITE
if eRSITE.Text='' then begin
   alarmmsg:=alarmmsg+'- не задано имя RSITE'+#13#10;
   datachek:=false;
end;
// проверка BSC
if cbBSC.ItemIndex=-1 then begin
   alarmmsg:=alarmmsg+'- не задан BSC'+#13#10;
   datachek:=false;
end;
// проверка LAI
if cbLAI.ItemIndex=-1 then begin
   alarmmsg:=alarmmsg+'- не задан LAI'+#13#10;
   datachek:=false;
end;
// проверка RO
if eRO.Text='' then begin
   alarmmsg:=alarmmsg+'- не задано RO'+#13#10;
   datachek:=false;
end;
// проверка EA
if eEA.Text='' then begin
   alarmmsg:=alarmmsg+'- не задано EA'+#13#10;
   datachek:=false;
end;
// проверка общего  BSIC
if cbGenBSIC.Checked=true then
    if eBSIC.Text='' then begin
       alarmmsg:=alarmmsg+'- не задан общий BSIC'+#13#10;
       datachek:=false;
     end;
// проверка параметров по секторам
// проверка частот
for i:=1 to 6 do
begin
  if LSector[i].Active=true then
    begin
      ...
      if LSector[i].CountTRU<>nc then begin
          alarmmsg:=alarmmsg+'- недостаточно частот в секторе "'+LSector[i].Name+'"'+#13#10;
          datachek:=false;
      end;
    end;
end;
    ...
    begin
      if cbGenBSIC.Checked=false then
        if LSector[i].Freq.BSIC='' then begin
            alarmmsg:=alarmmsg+'- не задан BSIC в секторе "'+LSector[i].Name+'"'+#13#10;
            datachek:=false;
        end;
    end;
end;
  ...
  if LSector[i].Active=true then
    begin
        if LSector[i].EDGE.EDGETS='' then begin
            alarmmsg:=alarmmsg+'- не заданы TS под EDGE в секторе "'+LSector[i].Name+'"'+#13#10;
            datachek:=false;
        end;
    end;
end;
...
var
  itmp: integer;
begin
// обновление вкладки частоты
  // обновлени вкладка Сектора/Частоты конфигурация
  ...
  eConfF2.Text:=IntToStr(LSector[6].CountTRU);
    ...
    // настраивам форму согласно конфигурации
...
// RLLOC
itmp:=0;
if LSector[1].TMA=true then itmp:=itmp-LOptions.BS_TMA;       // МШУ
...
if LSector[1].Diverity='AB' then eBSRMINA.Text:=IntToStr(LOptions.BSRXMIN+2)  // сдвоенный прием
...
// Расчет RLLOC
itmp:=0;
if LSector[2].TMA=true then itmp:=itmp-LOptions.BS_TMA;       // МШУ
...
if LSector[2].Diverity='AB' then eBSRMINB.Text:=IntToStr(LOptions.BSRXMIN+2)   // сдвоенный прием
...
// RLLOC
itmp:=0;
if LSector[3].TMA=true then itmp:=itmp-LOptions.BS_TMA;       // МШУ
...
if LSector[3].Diverity='AB' then eBSRMINC.Text:=IntToStr(LOptions.BSRXMIN+2)   // сдвоенный прием
...
// RLLOC
itmp:=0;
if LSector[4].TMA=true then itmp:=itmp-LOptions.BS_TMA;       // МШУ
...
if LSector[4].Diverity='AB' then eBSRMIND.Text:=IntToStr(LOptions.BSRXMIN+2)   // сдвоенный прием
...
// RLLOC
itmp:=0;
if LSector[5].TMA=true then itmp:=itmp-LOptions.BS_TMA;       // МШУ
...
if LSector[5].Diverity='AB' then eBSRMINE.Text:=IntToStr(LOptions.BSRXMIN+2)  // сдвоенный прием
...
// RLLOC
itmp:=0;
if LSector[6].TMA=true then itmp:=itmp-LOptions.BS_TMA;       // МШУ
...
if LSector[6].Diverity='AB' then eBSRMINF.Text:=IntToStr(LOptions.BSRXMIN+2)   // сдвоенный прием
...
// считываем конфигурацию с формы
...
begin
  ...
  // обнуляем привязку трансивером к секторам
  for n:=0 to 11 do
  ...
  // активируем сектора, определяем конфигурацию
  for n:=0 to 11 do
...
// считываем данные по транспорту
...
begin
  ...
  begin
    // пропускаем команды для SIU, если используем E1
    ...
    if strCICLE='TRUE' then
        begin
          strtmp:=''; sumstr:='';
            for i:=1 to 6 do  // цикл по секторам
              begin
                if LSector[i].Active=true then
                  begin
  ...
  Case num_step of
    ...
    2: begin
       ...
       if strCICLE='TRUE' then
                ...
                begin
                  ...
                  if LSector[i].CountTRU=1 then
                    begin
                      ...
                      // исключим команды Power Saving для TRU=1;
                      if ((cmd='RLSVI') or (cmd='RLSVC')) then strtmp:='';

                       // исключим команды BTS Power control для TRU=1;
                  ...
                  // Кодеки
                  if LSector[i].CountTRU<=LCodec.RowCount then // поддерживаем только до 4-х TRU в сектор
                  begin
                 ...
                 if strtmp<>'' then sumstr:=sumstr+strtmp+#13#10; // ислюлим вывод лишних строк
                end; //if
            end;  //for
        ...
        // обработка не повторяющихся строк 2-го блока
        end else begin
          res:=strCMD;
        end;
    end; // block 2
        ...
        begin
          ...
          if strCICLE='TRUE' then begin
           ...
           for i:=0 to LRbs[cbRBSType.ItemIndex+1].countTRU-1 do
            begin
              if LRbs[cbRBSType.ItemIndex+1].TRU[i]=0 then continue;  // исключаем не подключеные TRX
              if (nTRU<=0) and (nSC<4) then begin      // распределяем SC
                nTRU:=3;
                nSC:=nSC+1;
              end;
              // обрабатываем блок команд
          ...
          res:='-1'; // вернем результат, в противном случае вывода не будет
          cmdTMPList.Free;
        end else res:='';
       end; // block 6
                ...
                begin
                  if LSector[i].Active=true then
                    begin
                      strtmp:=StringReplace(strCMD, '{SITE}X', eSITE.Text+LSector[i].Name, [rfReplaceAll, rfIgnoreCase]);
                      strtmp:=StringReplace(strtmp, '{TG}', eTG.Text, [rfReplaceAll, rfIgnoreCase]);
                      if strtmp<>'' then sumstr:=sumstr+strtmp+#13#10; // исключим вывод лишних строк
                    end;
                end;  //for
              delete(sumstr,length(sumstr)-1,2);
              res:=sumstr;
    ...
    10: begin
        if LTransp.TransType='E1' then begin
         if strCICLE='TRUE' then
            begin
              sumstr:='';
              if LTransp.cbTypeRBL1='RBL2' then begin   // порт А
                // TCH
                ...
                sumstr:=sumstr+strtmp+';'+#13#10;
                // EDGE
                if pos('{RBL}',strCMD)=0 then begin    // if - затычка
              ...
              if LTransp.cbTypeRBL2='RBL2' then begin  // порт B/C/D
...
begin
  If CBTransType.ItemIndex=1 then
    begin
      ...
      If cbSIG.Text<>'SCCONC' then cbSIG.ItemIndex:=0; // забываю переключаться!
    end else begin
      GPTransE1.Visible:=true;
      GPTransSIU.Visible:=false;
      If cbSIG.Text='SCCONC' then cbSIG.ItemIndex:=1; // забываю переключаться!
    end;
end;

// обработчик команд скрипта УДАЛЕНИЯ
...
begin
 ...
 // res:='';
  ...
  begin
    ...
    // пропускаем команды для E1, если используем SIU
  ...
  Case num_step of
    ...
    2: begin
       ...
       if strCICLE='TRUE' then
        ...
        // обработка не повторяющихся строк блока
        end else begin
...
begin
  ...
  // загружаем настроек из INI файла
  LoadINI;

  // загружаем данные по LAI
  LoadCSVFile('lai.ini',';');
  ...
  // загружаем данные по параметрам кодеков
  ...
  // загружаем данные по версиям ПО
...
procedure TFMain.N6Click(Sender: TObject);  
  var cStep:integer;
       ...
       datachek: boolean;
begin
// считываем конфигурацию оборудования
UpdateDataForm;
...
// цикл по блокам скрипта
  for cStep:=1 to 13 do
    begin
      ...
      if flTYPECOM='T' then begin  // целый блок!
        cmdList.Clear;
